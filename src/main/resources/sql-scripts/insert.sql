...
INSERT INTO Person(SELECT 'borg' AS username, 'greta@borg.kth.se' AS email, 'Greta' AS fname, 'wl9nk23a' AS password, '19740212-1521' AS ssn, 'Borg' AS surname FROM Person WHERE 'borg' = username AND 'greta@borg.kth.se' = email AND 'Greta' = fname AND 'wl9nk23a' = password AND '19740212-1521' = ssn AND 'Borg' = surname HAVING count(*)=0)
INSERT INTO Person(SELECT 'strand' AS username, 'per@strand.kth.se' AS email, 'Per' AS fname, 'wl9nk23a' AS password, '19671212-1211' AS ssn, 'Strand' AS surname FROM Person WHERE 'strand' = username AND 'per@strand.kth.se' = email AND 'Per' = fname AND 'wl9nk23a' = password AND '19671212-1211' = ssn AND 'Strand' = surname HAVING count(*)=0)
